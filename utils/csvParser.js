...
/**
 * Parses a CSV file and converts rows into Mongoose Question objects.
 * @param {string} filePath - Absolute path to the CSV file.
 * @param {string} createdById - User ID of the admin importing the questions.
 * @returns {Promise<Array>} - List of validated Question objects.
 */
const parseQuestionsCSV = (filePath, createdById) => {
  return new Promise((resolve, reject) => {
    ...
    fs.createReadStream(filePath)
      ...
      .on('data', (row) => {
        ...
        let correctAns = row.correctAnswer || row.correct || row.Answer || row['Correct Answer'] || row.correct_answer;
        if (correctAns) correctAns = correctAns.trim().toUpperCase();

        const marksVal = parseInt(row.marks || row.Marks || 1, 10);
        const negMarksVal = parseFloat(row.negativeMarks || row.negative_marks || row['Negative Marks'] || 0);
        const subjectVal = row.subject || row.Subject || row.category || 'General';

        if (text && optA && optB && optC && optD && ['A', 'B', 'C', 'D'].includes(correctAns)) {
          questionsList.push({
            text: text.trim(),
            options: {
              A: optA.trim(),
